function [z,res_p,res_d] = sunsal(M,y,varargin)   %M为A
...
%---------------------------------------------
% least squares constrained (sum(x) = 1) 和为一最小二乘
%---------------------------------------------
SMALL = 1e-12;
B = ones(1,p);
a = ones(1,N);
...
if  strcmp(addone,'yes') && strcmp(positivity,'no') 
    ...
    if rcond(F) > SMALL   
        ...
        IF = inv(F);
        z = IF*M'*y-IF*B'*inv(B*IF*B')*(B*IF*M'*y-a);   %此步骤为实现
        % primal and dual residues
        res_p = 0;
        res_d = 0;
        return
...
%%
%---------------------------------------------
%  Constants and initializations 常量与初始化
%---------------------------------------------
mu_AL = 0.01;
...
while (i <= AL_iters) && ((abs (res_p) > tol1) || (abs (res_d) > tol2)) 
    ...
    if mod(i,10) == 1
        ...
        if  strcmp(verbose,'yes')
            fprintf(' i = %d, obj= %.2f, res_p = %f, res_d = %f\n',i,norm(y-M*x,'fro'),res_p,res_d)
        end
        % update mu
        if res_p > 10*res_d
            mu = mu*2;
